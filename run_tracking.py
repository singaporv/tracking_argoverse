...
def run_tracking(
    ...
    # for ind_frame in range(10):
        ...
        #  visualizing type of data
        # print (pc_raw0.shape, pc_raw_roi.shape, len(pc_segments))
        # print (pc_segments[0].shape)
        # print (type(pc_raw0) , " ", type(pc_raw_roi), " ", type(pc_segments))

        #function for visualizing detection result
        # and saving results
        np.save("custom/data/pc_raw_" + str(ind_frame), pc_raw0)
        np.save("custom/data/pc_roi_" + str(ind_frame), pc_raw_roi)
        import pickle
        pickle.dump(pc_segments, open( "custom/data/pc_seg_" + str(ind_frame)+ ".p", "wb"))
        # sys.exit(0)
        ...
        if len(tracker.tracks) == 0:
            # ego_R and ego_t are ego translation and rotation 
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--path_dataset", type=str, help="root path for input dataset", 
        default="/data"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--path_output", type=str, help="root path for output files", 
