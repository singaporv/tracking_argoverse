...
class Tracker:
    ...
    ind_current_frame = 0
    city_name = 5
    count_fail_th = 5 # miss threshold 
    path_output = ""
    ...
    def initialize_tracks(self, pc_segments, city_R_egovehicle, city_t_egovehicle):
        ...
        # print ("Number of tracks are: ", len(self.tracks[0]))
        # sys.exit(0)
    ...
    def find_matches(self, pc_candidates):
        ...
        """
        # print ("Initial track number: ",len(self.tracks[0]))
        tracks = self.tracks
        ind_current_frame = self.ind_current_frame # current frame number
        tracked_id = list(tracks[ind_current_frame].keys())
        print ("Current number of tracks: ", len(tracked_id))
        print ("Current number of dets: ", len(pc_candidates))

        # contains 0s and 1s --> telling if the current track is matched
        track_matched = np.zeros(len(tracked_id))

        # this stores id of the tracks that is matched to the particular pc_candidate
        pc_matched_id = -np.ones(len(pc_candidates))
        match_label_ind = -np.ones(len(tracked_id)) # not used anywhere
        ...
        for ii in range(len(pc_candidates)):
            ...
            # loop over all the open tracks
            for iii in range(len(tracked_id)):

                # Condition -->  track has already been matched or track has expired 
                if (
                    track_matched[iii] == 1
                ...
                if (np.linalg.norm(diff_parallel) < 1.0
                    ...
                    is_close = True # bool flag for being close (track and dets)
                ...
                # condition --> to check if this track is the closest
                if is_close or overlap:
                    if dist_center < dist_min:
                        ind_match_min = iii
                        dist_min = dist_center

            # condition that there has been a match
            if ind_match_min != -1:
                track_matched[ind_match_min] = 1
                pc_matched_id[ii] = ind_match_min # associate pc_candidate with the track num
        ...
        # print ("Point cloud - track matched ID:", self.pc_matched_id)

        # print ("Final track number: ",len(self.tracks[0]))
        # sys.exit(0)
    ...
    def update(self, pc_candidates, city_R_egovehicle, city_t_egovehicle):
        ...
        # update frame index of current frame
        ...
        for ii in range(len(pc_matched_id)):
            ...
            # match found, update existing track!
                ...
                pc_seg = pc_candidates[ii] # this is the point cloud 
        ...
        # handling the tracks which were not able to be associated with the pc_candidates
        for ii in range(len(tracked_id)):
            
            # if this track id has been matched with the pc_candidate
            if ii in pc_matched_id:
                continue

            print("Missing track object ", tracked_id[ii])

            #Remove objects that were not detected for a while
            ...
            # assumes it to be in the same place   (previous frame)
            if self.motion_model == "static":
                tracks[ind_current_frame][tracked_id[ii]]["x"] = tracks[
                    ind_current_frame - 1
                ][tracked_id[ii]]["x"]

            # adding velocity (from previous model) in x direction --> constant velocity model
    ...
    def add_new_track(self, pc_new, track_id_new, city_R_egovehicle, city_t_egovehicle):
        ...
        tracks_frame[track_id_new]["count_fail"] = 0 # miss threshold
        tracks_frame[track_id_new]["bbox"] = bbox_new
        tracks_frame[track_id_new]["count"] = 0 # hit threshold
...
class Detector:
    ...
    def get_candidate_segments(
        ...
        # this implementation doesn't use maskrcnn - point removal
        if not self.use_maskrcnn:
            ...
            print ("Printing after and before segments (maskrcnn): " \
                , len(is_car_maskrcnn), len(pc_segments))
            ...
            if save_results:
                ...
                # here num_imgs = camera numbers = 7
                for ind_cam in range(num_imgs):
                    if self.dataset_name == "Argoverse":
                        camera_name = self._RING_CAMERA_LIST[ind_cam]
                    ...
                    for ii in range(len(pc_segments)):
                        ...
                        for offset1 in range(-1, 2):
                            for offset2 in range(-1, 2):
                                img[ uv_valid[:, 1] + offset1, uv_valid[:, 0] + offset2, :,
                                ] = (
