...
def pc_segmentation_dbscan_multilevel(
    ...
    # print ("Final segments: ", len(pc_segs))
...
def pc_segmentation_dbscan(
    ...
    # pc_segs contains list of segments --> which includes all the point in that segment
    pc_segs = [None] * num_seg
    visited = [None] * num_seg
    # print ("Number of segements: ", num_seg)
    _out = 0
    _in = 0
    ...
    # print ("Outliers: ", _out, "Inliers: ", _in)

    # heiuristic based method to find it the given segment could be a car or not
    ind_keep = []
...
def pc_remove_ground(
    ...
    ground height and threshold are both same = 0.3
    pc_np --> np array of shape  = [num, 3] --> x, y, z
    """

    ground_removal_method = "plane_fitting"
    # print ("Shape of pc: ", pc_np.shape)

    if ground_removal_method == "plane_fitting":
        view_range = 100  # only process point cloud within 100m
        # below code returns indices - that satisfies the condition
        inds = (
            (pc_np[:, 0] < view_range)
            & (pc_np[:, 1] < view_range)
            & (pc_np[:, 0] > -view_range)
        ...
        pc_np = pc_np[inds]
        # print ("Shape of filtered points", pc_np.shape)
        plane_range_th = 1000
        ...
        # print ("ground plane pc", pc_ground.shape)
        # below code sends every 10th point to the is_plane function 
        is_pl, normal, d = is_plane(pc_ground[::10, :], 0.3)
        if normal[2] < 0:
            normal = -normal
            d = -d
    ...
    # this simply return all points that have height greater than threshold
...
def is_plane(pc, th):
    """
    Do plane detection
    Alternate approach would be using ransac --> but it will be slow
    but it would be better since lot of outliers will be there
    """

    # print ("Every 10th point in pc" , pc.shape)
    sample_size = 100
    if len(pc) > sample_size:
        random.sample(np.arange(len(pc)).tolist(), sample_size)

    # this is the average (x,y,z) values of all the points 
    ...
    u_norm = vh[2, :] # null space is orthogonal to the pointcloud
...
def show_pc_segments(pc_all, pc_all2, pc_segments):
   ...
   mlab.savefig(pc_axis, size=None, figure=None, magnification='auto')
   mlab.show()


def is_car(pc, min_point_num):
    """
    This function takes in point cloud of each segment 
    Shape heuristics for car detection
    it tells is a particular pc segment could be a car or not
    using --> 
    """
    ...
    # lower bound on height of the car
    ...
    # only xy coords are passed to find the bounding 
    bbox_min = MinimumBoundingBox.MinimumBoundingBox(pc[:, 0:2])
    l01 = bbox_min.length_parallel
    l02 = bbox_min.length_orthogonal
    area = l01 * l02

    # this puts bounds on area, length, width of the car
    if (
        (area < 1.5 or area > 20)
        or ((l01 > 6 or l02 > 6))
...
def check_pc_bbox_overlap(pc1, bbox2):
    ...
    returns a bool value if 2 overlap is positive
    """
    # print (len(pc1)) # this is total number of points
    bbox = bbox2.as_3d_bbox()  
    b2_c = Polygon(bbox[idx_bbox, :])
    b1 = get_pc_bbox(pc1)
    b1_c = Polygon(b1)
    inter_area = b1_c.intersection(b2_c).area
    # print (inter_area)  # gives intersection area in float
